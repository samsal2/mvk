...
{
  [[nodiscard]] bool isExtPresent( char const * ExtName, std::span<VkExtensionProperties const> const Exts ) noexcept
  {
  ...
  [[nodiscard]] bool chkExtSup( VkPhysicalDevice PhysicalDevice, std::span<char const * const> DeviceExtensions ) noexcept
  {
    ...
    vkEnumerateDeviceExtensionProperties( PhysicalDevice, nullptr, &ExtCount, std::data( Exts ) );
  ...
  [[nodiscard]] std::optional<std::pair<uint32_t, uint32_t>> queryFamiliyIdxs( VkPhysicalDevice const PhysicalDevice,
                                                                               VkSurfaceKHR const     Surface )
  {
    auto QueueFamilyCount = uint32_t( 0 );
    vkGetPhysicalDeviceQueueFamilyProperties( PhysicalDevice, &QueueFamilyCount, nullptr );

    auto QueueFamilyProps = std::vector<VkQueueFamilyProperties>( QueueFamilyCount );
    vkGetPhysicalDeviceQueueFamilyProperties( PhysicalDevice, &QueueFamilyCount, std::data( QueueFamilyProps ) );
  ...
  [[nodiscard]] VkPresentModeKHR choosePresentMode( VkPhysicalDevice PhysicalDevice, VkSurfaceKHR Surface ) noexcept
  {
    ...
    vkGetPhysicalDeviceSurfacePresentModesKHR( PhysicalDevice, Surface, &PresentModeCount, std::data( PresentModes ) );
