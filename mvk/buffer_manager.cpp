...
static constexpr VkBufferUsageFlags get_usage(buffer_type type) noexcept;

} // namespace detail

buffer_manager::buffer_manager(types::device * const device, types::physical_device const physical_device, types::command_pool * const command_pool, types::queue graphics_queue, buffer_type const type, types::device_size const default_size) : device_(device), physical_device_(physical_device), command_pool_(command_pool), graphics_queue_(graphics_queue), type_(type)
{
...
// a buffer each time
[[nodiscard]] buffer_manager::allocation buffer_manager::map(utility::slice<std::byte> const src)
{
        ...
        detail::stage(*device_, physical_device_, graphics_queue_, *command_pool_, current_buffer(), src, offset);
        return {current_buffer(), update_current_offset(size)};
}

void buffer_manager::create_new_buffers_and_memories(types::device_size const size) noexcept
{
        ...
        auto const requirements = detail::query<vkGetBufferMemoryRequirements>::with(types::parent(current_buffer()), types::get(current_buffer()));
        ...
        auto const memory_type_index = detail::find_memory_type(types::get(physical_device_), requirements.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
        ...
        for (size_t i = 0; i < std::size(buffers_); ++i)
        {
                ...
                vkBindBufferMemory(types::parent(buffers_[i]), types::get(buffers_[i]), types::get(buffers_memory_), current_size);
        }
}

void buffer_manager::add_current_buffers_and_memories_to_garbage()
{
...
void buffer_manager::clear_garbage()
{
        current_garbage_index_ = (current_garbage_index_ + 1) % std::size(garbage_buffers_);
        current_garbage_buffers().clear();
        current_garbage_memories().clear();
}

void buffer_manager::next_frame()
{
...
static constexpr VkBufferUsageFlags get_usage(buffer_type const type) noexcept
{
        switch (type)
        {
...
shader_stage_builder & shader_stage_builder::add_stage(types::shader_module shader_module, VkShaderStageFlagBits const stage) noexcept
{
