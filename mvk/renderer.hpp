...
private:
        ...
        void recreate_after_framebuffer_change();

        void begin_draw();

        void end_draw();

        void basic_draw(utility::slice<std::byte> vertices, utility::slice<std::byte> indices, utility::slice<std::byte> pvm);

        [[nodiscard]] pvm create_test_pvm();
