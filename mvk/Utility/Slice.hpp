...
namespace Detail {
template <typename T, size_t Size> struct SliceStorage {
  constexpr SliceStorage() noexcept = default;

  explicit SliceStorage(T *const Ptr,
                        [[maybe_unused]] size_t const Count) noexcept
      : Data(Ptr) {}
...
template <typename T> struct SliceStorage<T, DynamicExtent> {
  constexpr SliceStorage() noexcept = default;

  explicit SliceStorage(T *const Ptr, size_t const Size) noexcept
      : Data(Ptr), Extent(Size) {}
...
template <typename It, typename Elem,
          typename Val = Detail::ValueTypeFromIt<It>>
...
template <typename T, size_t Extent = DynamicExtent> class Slice {
public:
  ...
  constexpr explicit(Extent != DynamicExtent)
      Slice(It Begin, size_type Count) noexcept
  ...
  */
  ...
  template <typename U, size_t N>
  requires ConvertibleAsArrayTo<U, element_type>
  constexpr Slice(std::array<U, N> const &Arr) noexcept
      : Storage(std::data(Arr), std::size(Arr)) {
    MVK_VERIFY(Extent == DynamicExtent || Extent == size());
  }
  ...
  requires WithDataAndSize<Container> &&
      CompatibleWithElement<Container, element_type>
  constexpr Slice(Container &Contr) noexcept
      : Storage(std::data(Contr), std::size(Contr)) {
    MVK_VERIFY(Extent == DynamicExtent || Extent == size());
  }
  ...
  [[nodiscard]] constexpr pointer data() const noexcept { return Storage.Data; }

  [[nodiscard]] constexpr iterator begin() const noexcept { return &data()[0]; }
  ...
  operator[](size_t const Idx) const noexcept {
    MVK_VERIFY(Idx < size());
    return data()[Idx];
  }
  ...
  subSlice(size_type const Off,
           size_type const Count = DynamicExtent) const noexcept {
...
template <typename T> Slice(T &) -> Slice<Detail::ValueTypeFromData<T>>;

template <typename Container> struct NotSliceImpl : std::false_type {};
template <typename T, size_t Extent>
struct NotSliceImpl<Slice<T, Extent>> : std::true_type {};

template <typename Container>
concept NotSlice = requires {
  requires NotSliceImpl<Container>::value;
};
...
/*
template <typename Container>
requires WithDataAndSize<Container> && NotSlice<Container>
[[nodiscard]] constexpr Slice<std::byte const> as_bytes(Container const &Src)
noexcept { return as_bytes(Slice(Src)); }
*/
