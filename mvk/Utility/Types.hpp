...
forceCastToByte(T const *Data) noexcept {
...
static constexpr inline auto IsConvertibleAsArrayTo =
...
struct IsCompatWithElemImpl<
    Container, Elem,
    std::void_t<decltype(std::data(std::declval<Container>()))>>
    : std::bool_constant<
          IsConvertibleAsArrayTo<Detail::ValueTypeFromData<Container>, Elem>> {
};

template <typename Container, typename Element>
static constexpr inline bool IsCompatibleWithElement =
    IsCompatWithElemImpl<Container, Element>::value;

template <typename It>
using GetItCategory = typename std::iterator_traits<It>::iterator_category;

template <typename It>
static constexpr inline auto IsRandomAccess =
    std::is_same_v<GetItCategory<It>, std::random_access_iterator_tag>;

namespace Detail {
template <typename It>
[[nodiscard]] constexpr decltype(&(*std::declval<It>()))
unwrapIt(It const Iter) noexcept {
  return &(*Iter);
}

template <typename It>
using ValueTypeFromIt =
    std::remove_pointer_t<decltype(unwrapIt(std::declval<It>()))>;

} // namespace Detail

template <typename T> using Uncvref = std::remove_cvref_t<T>;

template <typename T> static constexpr inline auto AlwaysFalse = false;
