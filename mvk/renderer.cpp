...
void renderer::init_vulkan()
{
        ...
        physical_device_ = detail::choose_physical_device(types::parent(surface_), types::get(surface_), device_extensions);

        auto const queue_indices_result = detail::query_family_indices(types::get(physical_device_), surface_.get());
...
{
        auto const family_indices = std::array{graphics_queue_index_, present_queue_index_};
        ...
        auto const swapchain_create_info = [this, &family_indices]
        {
                ...
                auto const format           = detail::choose_surface_format(types::get(physical_device_), types::get(surface_), detail::default_format_checker);
                auto const capabilities     = detail::query<vkGetPhysicalDeviceSurfaceCapabilitiesKHR>::with(types::get(physical_device_), types::get(surface_));

                auto const present_mode = detail::choose_present_mode(types::get(physical_device_), surface_.get());
        ...
        swapchain_images_ = detail::query<vkGetSwapchainImagesKHR>::with(types::get(device_), types::get(swapchain_));
        ...
        auto const add_image_view = [this, &swapchain_create_info](auto const image)
        {
                ...
                swapchain_image_views_.emplace_back(types::get(device_), image_view_create_info);
        };

        std::for_each(std::begin(swapchain_images_), std::end(swapchain_images_), add_image_view);
        ...
        depth_image_memory_ = detail::create_device_memory(physical_device_, depth_image_, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
        ...
        depth_image_view_ = types::image_view(types::get(device_), depth_image_view_create_info);

        detail::transition_layout(device_, graphics_queue_, command_pool_, depth_image_, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_DEPTH_STENCIL_ATTACHMENT_OPTIMAL, depth_image_create_info.mipLevels);
}

void renderer::preload_stuff()
{
        ...
        std::tie(texture_, width_, height_) = detail::load_texture("../../assets/viking_room.png");
        ...
        image_memory_ = detail::create_device_memory(physical_device_, image_, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
        detail::transition_layout(device_, graphics_queue_, command_pool_, image_, VK_IMAGE_LAYOUT_UNDEFINED, VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL, image_create_info.mipLevels);
        detail::stage(device_, physical_device_, graphics_queue_, command_pool_, image_, utility::as_bytes(texture_), width_, height_);
        detail::generate_mipmaps(device_, graphics_queue_, command_pool_, image_, width_, height_, image_create_info.mipLevels);
        ...
        vertex_buffer_manager_ = buffer_manager(&device_, physical_device_, &command_pool_, graphics_queue_, buffer_type::vertex);
        index_buffer_manager_  = buffer_manager(&device_, physical_device_, &command_pool_, graphics_queue_, buffer_type::index);
}

void renderer::init_main_renderpass()
{
        auto const format = detail::choose_surface_format(types::get(physical_device_), types::get(surface_), detail::default_format_checker).format;
...
void renderer::init_framebuffers()
{
        ...
        std::for_each(std::begin(swapchain_image_views_), std::end(swapchain_image_views_), add_framebuffer);
}
...
void renderer::init_descriptors()
{
        ...
        {
                auto const [descriptor_set_binding_data, descriptor_set_binding_size] = utility::bind_data_and_size(descriptor_set_bindings);
        ...
        auto const images_count = static_cast<uint32_t>(std::size(swapchain_images_));
        ...
        for (auto i = size_t(0); i < images_size; ++i)
        {
                ...
                auto uniform_buffer_memory = detail::create_device_memory(physical_device_, uniform_buffers_.back(), VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT | VK_MEMORY_PROPERTY_HOST_COHERENT_BIT);

                mapped_datas_.push_back(detail::map_memory(uniform_buffer_memory, sizeof(pvm)));
        ...
        for (auto i = size_t(0); i < images_size; ++i)
        {
                ...
                vkUpdateDescriptorSets(types::get(device_), static_cast<uint32_t>(std::size(descriptor_writes)), std::data(descriptor_writes), 0, nullptr);
        }
}
...
void renderer::init_sync()
{
        ...
        {
                image_available_semaphores_[i] = types::semaphore(device_.get(), semaphore_create_info);
                render_finished_semaphores_[i] = types::semaphore(device_.get(), semaphore_create_info);
                frame_in_flight_fences_[i]     = types::fence(device_.get(), fence_create_info);
        }
...
void renderer::recreate_after_framebuffer_change()
{
...
void renderer::run()
{
        while (!window_.should_close())
        {
                ...
                basic_draw(utility::as_bytes(vertices_), utility::as_bytes(indices_), utility::as_bytes(pvm));
...
{
        auto const & image_available_semaphore = image_available_semaphores_[current_frame_index_];

        auto const current_image_index = detail::next_swapchain_image(types::get(device_), types::get(swapchain_), types::get(image_available_semaphore), nullptr);
        ...
        {
                auto const [clear_values_data, clear_values_size] = utility::bind_data_and_size(clear_values);
        ...
        vkBeginCommandBuffer(types::get(command_buffers_)[current_image_index_], &command_buffer_begin_info);
        vkCmdBeginRenderPass(types::get(command_buffers_)[current_image_index_], &render_pass_begin_info, VK_SUBPASS_CONTENTS_INLINE);
}
void renderer::basic_draw(utility::slice<std::byte> const vertices, utility::slice<std::byte> const indices, utility::slice<std::byte> const pvm)
{
        auto const [vertex_buffer, vertex_offset] = vertex_buffer_manager_.map(vertices);
        auto const [index_buffer, index_offset]   = index_buffer_manager_.map(indices);
        ...
        vkCmdBindPipeline(types::get(command_buffers_)[current_image_index_], VK_PIPELINE_BIND_POINT_GRAPHICS, types::get(pipeline_));
        vkCmdBindVertexBuffers(types::get(command_buffers_)[current_image_index_], 0, 1, &types::get(vertex_buffer), &vertex_offset);
        vkCmdBindIndexBuffer(types::get(command_buffers_)[current_image_index_], types::get(index_buffer), index_offset, VK_INDEX_TYPE_UINT32);
        vkCmdBindDescriptorSets(types::get(command_buffers_)[current_image_index_], VK_PIPELINE_BIND_POINT_GRAPHICS, types::get(pipeline_layout_), 0, 1, &types::get(descriptor_sets_)[current_image_index_], 0, nullptr);
        vkCmdDrawIndexed(types::get(command_buffers_)[current_image_index_], static_cast<uint32_t>(std::size(indices_)), 1, 0, 0, 0);
}
...
void renderer::end_draw()
{
        ...
        auto & image_in_flight_fence = image_in_flight_fences_[current_image_index_];

        if (image_in_flight_fence != nullptr)
        {
                vkWaitForFences(types::parent(*image_in_flight_fence), 1, &types::get(*image_in_flight_fence), VK_TRUE, std::numeric_limits<int64_t>::max());
        }
        ...
        auto const & image_available_semaphore = image_available_semaphores_[current_frame_index_];
        auto const & render_finished_semaphore = render_finished_semaphores_[current_frame_index_];

        detail::submit_draw_commands(graphics_queue_, types::get(command_buffers_)[current_image_index_], image_available_semaphore, render_finished_semaphore, *image_in_flight_fence);
        ...
        detail::present_swapchain(present_queue_, swapchain_, render_finished_semaphore, current_image_index_, check_present_result);
...
[[nodiscard]] float renderer::time() const noexcept
{
...
[[nodiscard]] pvm renderer::create_test_pvm()
{
        ...
        ubo.model = glm::rotate(glm::mat4(1.0F), current_time * turn_rate, glm::vec3(0.0F, 0.0F, 1.0F));
        ubo.view  = glm::lookAt(glm::vec3(2.0F, 2.0F, 2.0F), glm::vec3(0.0F, 0.0F, 0.0F), glm::vec3(0.0F, 0.0F, 1.0F));

        auto const ratio = static_cast<float>(extent_.width) / static_cast<float>(extent_.height);
