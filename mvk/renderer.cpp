...
renderer::init_framebuffers()
{
  ...
  std::for_each(std::begin(image_views), std::end(image_views),
                add_framebuffer);
}
...
renderer::load_mesh()
{
  ...
  for (size_t i = 0; i < std::size(command_buffers_.get()); ++i)
  {
    ...
    auto const clear_values =
        std::array{clear_color_value, clear_depth_value};
