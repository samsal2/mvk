...
renderer::init_vulkan()
{
  ...
  auto const validation_layers =
    vk_types::validation::validation_layers_data();
  ...
  auto const queue_create_info_count =
    static_cast<uint32_t>(queue_indices.first != queue_indices.second ? 2 : 1);

  auto const device_create_info = [validation_layers,
                                   &queue_create_info,
                                   &device_extensions,
  ...
  command_pool_ =
    vk_types::command_pool(device_.get(), command_pool_create_info);
}
...
renderer::init_swapchain()
{
  ...
  depth_image_view_ =
    vk_types::image_view(device_.get(), depth_image_view_create_info);
...
renderer::init_commands()
{
  ...
  command_buffers_ =
    vk_types::command_buffers(device_.get(), command_buffers_allocate_info);
}
...
renderer::init_descriptors()
{
  ...
  descriptor_set_layout_ = vk_types::descriptor_set_layout(
    device_.get(),
  ...
  descriptor_pool_ =
    vk_types::descriptor_pool(device_.get(), descriptor_pool_create_info);
  ...
  descriptor_sets_ =
    vk_types::descriptor_sets(device_.get(), descriptor_sets_allocate_info);
...
renderer::init_pipeline()
{
  ...
  pipeline_layout_ =
    vk_types::pipeline_layout(device_.get(), pipeline_layout_create_info);
...
renderer::init_sync()
{
  ...
  for (auto i = size_t(0); i < max_frames_in_flight; ++i)
  {
    image_available_semaphores_.at(i) =
      vk_types::semaphore(device_.get(), semaphore_create_info);
    render_finished_semaphores_.at(i) =
      vk_types::semaphore(device_.get(), semaphore_create_info);
    frame_in_flight_fences_.at(i) =
      vk_types::fence(device_.get(), fence_create_info);
  }
...
renderer::load_mesh()
{
  ...
  auto const [index_buffer, index_offset] =
    index_buffer_manager_.map(utility::as_bytes(indices_));
...
renderer::run()
{
  ...
  while (!window_.should_close())
  {
    ...
    auto const submit_info =
      [this, &wait_semaphores, &signal_semaphores, &wait_stages, current_index]
    {
