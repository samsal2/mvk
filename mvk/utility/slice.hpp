...
public:
    ...
    constexpr explicit(extent != dynamic_extent) slice(Iterator begin, size_type count) : storage_(detail::unwrap_iterator(begin), count)
    {
    ...
    template <typename Iterator>
    requires detail::valid_slice_iterator<Iterator, const_value_type>
    constexpr explicit(extent != dynamic_extent) slice(Iterator begin, Iterator end) : storage_(detail::unwrap_iterator(begin), std::distance(begin, end))
    {
        MVK_VERIFY(extent == dynamic_extent || extent == size());
    }
    ...
    */
    ...
    constexpr slice(std::array<U, N> & array) : storage_(std::data(array), std::size(array))
    {
        MVK_VERIFY(extent == dynamic_extent || extent == size());
    }
    ...
    template <typename U, size_t N>
    requires convertible_as_array_to<U, const_value_type>
    constexpr slice(std::array<U, N> const & array) : storage_(std::data(array), std::size(array))
    {
        MVK_VERIFY(extent == dynamic_extent || extent == size());
    }
    ...
    constexpr slice(Container & container) : storage_(std::data(container), std::size(container))
    {
        MVK_VERIFY(extent == dynamic_extent || extent == size());
    }
    ...
    template <typename U, size_t N>
    requires convertible_as_array_to<U, const_value_type>
    constexpr explicit(extent != dynamic_extent && N == dynamic_extent) slice(slice<U, N> other) : storage_(std::data(other), std::size(other))
    {
        MVK_VERIFY(extent == dynamic_extent || extent == size());
    }
    ...
    subslice(size_type const offset, size_type const count = dynamic_extent) const
    {
