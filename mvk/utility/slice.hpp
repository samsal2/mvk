...
template <typename T, size_t Extent> struct slice_storage
{
        ...
        explicit slice_storage(T const * const data, [[maybe_unused]] size_t const size) noexcept : data_(data)
        {
        }
...
template <typename T> struct slice_storage<T, dynamic_extent>
{
        ...
        explicit slice_storage(T const * const data, size_t const size) noexcept : data_(data), size_(size)
        {
        }
...
public:
        ...
        constexpr explicit(extent != dynamic_extent) slice(Iterator begin, size_type count) noexcept : storage_(detail::unwrap_iterator(begin), count)
        {
        ...
        template <typename Iterator>
        requires detail::valid_slice_iterator<Iterator, const_value_type>
        constexpr explicit(extent != dynamic_extent) slice(Iterator begin, Iterator end) noexcept : storage_(detail::unwrap_iterator(begin), std::distance(begin, end))
        {
                MVK_VERIFY(extent == dynamic_extent || extent == size());
        }
        ...
        */
        ...
        requires with_data_and_size<Container> && compatible_with_element<Container, const_value_type>
        ...
        template <typename U, size_t N>
        requires convertible_as_array_to<U, const_value_type>
        constexpr explicit(extent != dynamic_extent && N == dynamic_extent) slice(slice<U, N> other) noexcept : storage_(std::data(other), std::size(other))
        {
                MVK_VERIFY(extent == dynamic_extent || extent == size());
        }
        ...
        [[nodiscard]] constexpr slice<value_type> subslice(size_type const offset, size_type const count = dynamic_extent) const noexcept
        {
...
template <typename T> slice(T const &) -> slice<detail::value_type_from_data_t<T>>;

template <typename T, size_t E> [[nodiscard]] constexpr slice<std::byte, E * sizeof(T)> as_bytes(slice<T, E> const src) noexcept
{
        return {force_cast_to_byte(std::data(src)), E * sizeof(T)};
}

template <typename T> [[nodiscard]] constexpr slice<std::byte> as_bytes(slice<T> const src) noexcept
{
...
[[nodiscard]] constexpr slice<std::byte> as_bytes(Container const & src) noexcept
{
...
template <typename T>
requires trivial<T>
[[nodiscard]] constexpr slice<std::byte> as_bytes(T const & src) noexcept
{
