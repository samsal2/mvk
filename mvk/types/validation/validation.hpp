...
is_layer_present(VkLayerProperties const layer, utility::slice<char const *> const layers) noexcept
{
...
setup_debug_messenger(VkInstance const instance)
{
    if constexpr (!g_use_validation)
    {
        return nullptr;
...
static void
destroy_debug_messenger(VkInstance const instance, VkDebugUtilsMessengerEXT const messenger, VkAllocationCallbacks const * callback)
{
    if constexpr (!g_use_validation)
    {
        return;
...
[[nodiscard]] static constexpr VkDebugUtilsMessengerCreateInfoEXT const *
debug_create_info_ref() noexcept
{
    if constexpr (!g_use_validation)
    {
        return nullptr;
