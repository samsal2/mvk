...
static VKAPI_ATTR VKAPI_CALL VkBool32 debug_callback([[maybe_unused]] VkDebugUtilsMessageSeverityFlagBitsEXT severity, [[maybe_unused]] VkDebugUtilsMessageTypeFlagsEXT type, [[maybe_unused]] VkDebugUtilsMessengerCallbackDataEXT const * data, [[maybe_unused]] void * p_user_data)
{
...
static constexpr auto g_debug_create_info = []
{
        ...
        tmp.messageSeverity = VK_DEBUG_UTILS_MESSAGE_SEVERITY_VERBOSE_BIT_EXT | VK_DEBUG_UTILS_MESSAGE_SEVERITY_WARNING_BIT_EXT | VK_DEBUG_UTILS_MESSAGE_SEVERITY_ERROR_BIT_EXT;
        tmp.messageType     = VK_DEBUG_UTILS_MESSAGE_TYPE_GENERAL_BIT_EXT | VK_DEBUG_UTILS_MESSAGE_TYPE_VALIDATION_BIT_EXT | VK_DEBUG_UTILS_MESSAGE_TYPE_PERFORMANCE_BIT_EXT;
        tmp.pfnUserCallback = debug_callback;
        tmp.pUserData       = nullptr;
        return tmp;
}();

[[nodiscard]] static bool is_layer_present(VkLayerProperties const layer, utility::slice<char const *> const layers) noexcept
{
...
[[nodiscard]] static bool check_support() noexcept
{
        ...
        auto availables = std::vector<VkLayerProperties>(layer_count);
...
[[nodiscard]] static VkDebugUtilsMessengerEXT setup_debug_messenger(VkInstance const instance)
{
        ...
        auto const f = reinterpret_cast<PFN_vkCreateDebugUtilsMessengerEXT>(vkGetInstanceProcAddr(instance, "vkCreateDebugUtilsMessengerEXT"));
...
static void destroy_debug_messenger(VkInstance const instance, VkDebugUtilsMessengerEXT const messenger, VkAllocationCallbacks const * callback)
{
        if constexpr (!g_use_validation)
        {
                return;
        ...
        auto const f = reinterpret_cast<PFN_vkDestroyDebugUtilsMessengerEXT>(vkGetInstanceProcAddr(instance, "vkDestroyDebugUtilsMessengerEXT"));
        MVK_VERIFY(f);
        f(instance, messenger, callback);
}

[[nodiscard]] static constexpr utility::slice<char const *> validation_layers_data() noexcept
{
        if constexpr (!g_use_validation)
        {
                return {};
...
[[nodiscard]] static utility::slice<char const *> required_instance_extensions()
{
        if constexpr (!g_use_validation)
        {
                return {};
