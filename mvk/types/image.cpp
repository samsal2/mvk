...
image &
image::transition_layout(device const & device, command_pool const & command_pool, VkImageLayout const old_layout, VkImageLayout const new_layout)
{
    auto const [image_memory_barrier, source_stage, destination_stage] = [old_layout, new_layout, this]
    {
...
image &
image::stage(device const & device, command_pool const & command_pool, image::texture const & texture)
{
    auto const [staging_buffer, staging_buffer_memory] = detail::create_staging_buffer_and_memory(device, utility::as_bytes(texture));

    auto staging_command_buffer = detail::create_staging_command_buffer(device, command_pool);
    ...
    staging_command_buffer.begin(0, command_buffer_begin_info).copy_buffer_to_image({staging_buffer.get(), get(), VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL}, {&copy_region, 1}).end();
...
image &
image::generate_mipmaps(device const & device, command_pool const & command_pool, uint32_t width, uint32_t height)
{
    ...
    auto current_command_buffer = staging_command_buffer.begin(0, command_buffer_begin_info);
    ...
    for (auto i = uint32_t(0); i < (mipmap_levels() - 1); ++i)
    {
        ...
        current_command_buffer.blit_image({get(), VK_IMAGE_LAYOUT_TRANSFER_SRC_OPTIMAL}, {get(), VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL}, {&blit, 1}, VK_FILTER_LINEAR);
