...
public:
  ...
  struct allocation
  {
    std::reference_wrapper<vk_types::buffer> buffer;
    VkDeviceSize                             offset;
  };
...
private:
  ...
  // TODO(samuel): better names pls
  template <typename T>
  using dynamic_array = std::array<T, dynamic_buffer_count>;

  vk_types::device_memory         buffers_memory_;
  dynamic_array<vk_types::buffer> buffers_ = {};
  dynamic_array<VkDeviceSize>     offsets_ = {};

  template <typename T>
  using garbage_array = std::array<std::vector<T>, garbage_buffer_count>;

  garbage_array<vk_types::buffer>        garbage_buffers_          = {};
  garbage_array<vk_types::device_memory> garbage_buffers_memories_ = {};
