...
public:
        ...
        copy_buffer_to_image(VkBuffer source_buffer, VkImage destination_image, VkImageLayout destination_image_layout,
        ...
        single_command_buffer &
        pipeline_barrier(VkPipelineStageFlags source_stage, VkPipelineStageFlags destination_stage, VkDependencyFlags dependency_flags,
                         utility::slice<VkMemoryBarrier> memory_barriers, utility::slice<VkBufferMemoryBarrier> buffer_memory_barriers,
        ...
        single_command_buffer &
        draw(uint32_t vertex_count, uint32_t instance_count, uint32_t first_vertex, uint32_t first_instance) noexcept;

        single_command_buffer &
        begin_render_pass(VkRenderPassBeginInfo const & begin_info, VkSubpassContents subpass_contents) noexcept;
        ...
        single_command_buffer &
        blit_image(VkImage source_image, VkImageLayout source_image_layout, VkImage destination_image, VkImageLayout destination_image_layout,
                   utility::slice<VkImageBlit> region, VkFilter filter) noexcept;
