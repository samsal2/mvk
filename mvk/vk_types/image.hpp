#ifndef MVK_VK_TYPES_IMAGE_HPP_INCLUDE
...
class image : public detail::wrapper<VkImage, vkDestroyImage>
{
public:
        ...
        image &
        transition_layout(device const & device, command_pool const & command_pool, VkImageLayout old_layout, VkImageLayout new_layout);

        image &
        stage(device const & device, command_pool const & command_pool, image::texture const & texture);
        ...
        image &
        generate_mipmaps(device const & device, command_pool const & command_pool, uint32_t width, uint32_t height);
