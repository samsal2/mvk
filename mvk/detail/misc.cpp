...
choose_present_mode(VkPhysicalDevice const physical_device,
...
[[nodiscard]] bool
supports_surface(VkPhysicalDevice const physical_device,
                 VkSurfaceKHR const surface, uint32_t const index)
{
...
query_family_indices(VkPhysicalDevice const physical_device,
                     VkSurfaceKHR const surface)
{
  auto const queue_families =
      query<vkGetPhysicalDeviceQueueFamilyProperties>::with(physical_device);
...
choose_physical_device(
    VkInstance const instance, VkSurfaceKHR const surface,
    utility::slice<char const *> const device_extensions) noexcept
{
  auto const physical_devices =
      query<vkEnumeratePhysicalDevices>::with(instance);
      ...
      [device_extensions, surface](auto const physical_device)
  {
    ...
    return check_extension_support(physical_device, device_extensions) &&
           check_format_and_present_mode_availability(physical_device,
                                                      surface) &&
