...
create_device_memory(types::device const & device, Buffer const & buffer, VkMemoryPropertyFlags properties);
...
template <typename Buffer>
[[nodiscard]] types::device_memory
create_device_memory(types::device const & device, Buffer const & buffer, VkMemoryPropertyFlags const properties)
{
    auto const requirements      = buffer.memory_requirements();
    auto const memory_type_index = types::detail::find_memory_type(device.physical_device(), requirements.memoryTypeBits, properties);
